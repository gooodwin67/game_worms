...
export class TurnMachine {
  ...
  next() {
    ...
    for (const team of g.teams) if (team.worms.some(w => w.alive)) { survivors++; winner = team; }
    if (survivors <= 1) { g.winner = winner ? `${winner.name} побеждает!` : 'Ничья'; return; }
    do { this.team = (this.team + 1) % g.teams.length; } while (!g.teams[this.team].worms.some(w => w.alive));
    ...
    for (const worm of g.worms) if (worm.team === this.team) { worm.frozen = false; worm.speedBoost = false; worm.invisible = false; worm.poison = 0; worm.radiation = 0; }
  ...
  beginCharge() { if (this.state === TURN.WAITING_INPUT && this.game.active?.alive) { this.charge = 0; this.state = TURN.CHARGING_SHOT; } }
  release() {
    if (this.state !== TURN.CHARGING_SHOT) return;
    this.state = TURN.ACTION_RESOLVING;
    if (this.game.weapons.fire(this.weapon, this.charge) === false) { this.state = TURN.WAITING_INPUT; this.charge = 0; }
  }
  ...
  update(dt) {
    ...
    if (this.state === TURN.WAITING_INPUT || this.state === TURN.CHARGING_SHOT) {
      ...
      if (!this.game.active.alive) { this.shots = 0; this.settle(); return; }
      if (this.state === TURN.CHARGING_SHOT) this.charge = Math.min(1, this.charge + dt / 1.5);
      if (this.remaining === 0) { if (this.state === TURN.CHARGING_SHOT) this.release(); else { this.shots = 0; this.settle(); } }
    }
    if (this.state === TURN.SETTLING) {
      ...
      if (this.still >= 0.6) {
        if (this.weapon === 'shotgun' && this.shots > 0 && this.remaining > 0 && this.game.active.alive) this.state = TURN.WAITING_INPUT;
